...
async function processFile() {
  ...
  try {
    ...
    console.log("Compression completed, compressed size:", compressed.length);
    ...
    console.log("Encryption completed, encrypted size:", encrypted.length);
...
function generateQRChunks(data) {
  if (!data || !data.length) {
    throw new Error("No data provided to generate QR chunks");
  }

  console.log("Starting QR chunk generation with data size:", data.length);

  const chunkSize = 1000; // bytes per QR code
  const chunks = [];

  try {
    // Convert the entire Uint8Array to base64 first
    ...
    console.log("Generated", chunks.length, "QR chunks");
    ...
    console.error("Error in generateQRChunks:", error);
    throw new Error("Failed to generate QR chunks: " + error.message);
  }
}
...
function displayCurrentChunk() {
  ...
  document.getElementById("currentChunk").textContent = currentChunkIndex + 1;
